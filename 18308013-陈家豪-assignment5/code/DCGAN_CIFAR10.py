...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.convTranspose1 = nn.ConvTranspose2d(INT_DIMENSION, TRANS_CONV_SIZE*4, 4, 1, 0, bias=False)
        self.batchNorm1 = nn.BatchNorm2d(TRANS_CONV_SIZE*4)
        self.convTranspose2 = nn.ConvTranspose2d(TRANS_CONV_SIZE*4, TRANS_CONV_SIZE*2, 4, 2, 1, bias=False)
    ...
    def forward(self, x):
        out = self.convTranspose1(x) # (batch, INT_DIMENSION, 1, 1, 1)->(batch, TRANS_CONV_SIZE*4, 4, 4)
        out = self.batchNorm1(out)
        out = F.relu(out)
        out = self.convTranspose2(out) # (batch, TRANS_CONV_SIZE*4, 4, 4)->(batch, TRANS_CONV_SIZE*2, 8, 8)
        out = self.batchNorm2(out)
        out = F.relu(out)
        out = self.convTranspose3(out) # (batch, TRANS_CONV_SIZE*2, 8, 8)->(batch, TRANS_CONV_SIZE, 16, 16)
        out = self.batchNorm3(out)
        out = F.relu(out)
        out = self.convTranspose4(out) # (batch, TRANS_CONV_SIZE, 16, 16)->(batch, CHANNEL_NUM, 32, 32)
        out = torch.tanh(out)
        return out
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.conv3 = nn.Conv2d(TRANS_CONV_SIZE*2, CONV_SIZE*4, 4, 2, 1, bias=False)
        self.batchNorm2 = nn.BatchNorm2d(CONV_SIZE*4)
        self.conv4 = nn.Conv2d(CONV_SIZE*4, 1, 4, 1, 0, bias=False)

    def forward(self, x):
        out = self.conv1(x) # (batch, channels, 32, 32)->(batch, TRANS_CONV_SIZE, 16, 16)
        out = F.leaky_relu(out, 0.2, True)
        out = self.conv2(out) # (batch, TRANS_CONV_SIZE, 16, 16)->(batch, TRANS_CONV_SIZE*2, 8, 8)
        out = self.batchNorm1(out)
        out = F.leaky_relu(out, 0.2, True)
        out = self.conv3(out) # (batch, TRANS_CONV_SIZE*2, 8, 8)->(batch, TRANS_CONV_SIZE*4, 4, 4)
        out = self.batchNorm2(out)
        out = F.leaky_relu(out, 0.2, True)
        out = self.conv4(out) # (batch, TRANS_CONV_SIZE*4, 4, 4)->(batch, 1, 1, 1)
...
def main():
    ...
    # normalize
    train_images = (train_images-np.array([0.5,0.5,0.5]))/np.array([0.5,0.5,0.5])
    test_images = (test_images-np.array([0.5,0.5,0.5]))/np.array([0.5,0.5,0.5])
    trainDataset = MyDataset(train_images)
    testDataset = MyDataset(test_images)
    ...
    plt.imshow(np.transpose(utils.make_grid(realData.to(DEVICE)[:64], padding=2, normalize=True).cpu(),(1,2,0)))
    ...
    summary(discriminator, input_size=(CHANNEL_NUM,32,32))
